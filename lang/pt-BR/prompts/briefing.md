...
O vault fica em `{{vault}}`. Todo caminho deste briefing (as listas abaixo, os blocos, o log) é relativo a ele, e você abre e edita um arquivo pelo caminho absoluto dentro dele, `{{vault}}/<caminho>`, de onde quer que esta sessão tenha começado. Os comandos do kit abaixo já indicam o vault (`-C`), então funcionam de qualquer lugar: rode-os exatamente como estão escritos.
...
4. `{{kit}} propose "<resumo em uma linha>" --only <caminho> <caminho>`, listando todos os arquivos que você criou ou alterou, pelo caminho relativo ao vault, e nada além deles. Nunca use `--all`. Quem abre o pull request é o kit; você nunca faz commit, push nem merge por conta própria.
5. Só depois que o `propose` sair com 0, ou com 3 dizendo que o commit foi enviado, rode `{{kit}} questions answer <id>` para cada pergunta que o dono respondeu nesta sessão. Nunca antes, e nunca quando o `propose` não enviou nada.

Um comando do kit que sai com 75 foi recusado porque outro processo (uma rodada agendada da curadoria) está com o vault, e ele diz qual. Pare de escrever na hora e não rode mais nada. Diga ao dono que a rodada está com o vault, quais arquivos você escreveu que ainda não foram propostos, e os comandos exatos para rodar quando a rodada terminar: o `propose` acima com os caminhos, e depois `questions answer` para cada pergunta respondida. Nunca marque uma pergunta como respondida nesse caso.

Nunca termine o briefing com um arquivo escrito e não proposto sem dizer ao dono quais arquivos e o comando exato que os propõe.

...
9. Never count as a ritual:
   - an event from someone else's calendar, unless the configuration records who authorised reading the team's calendars (`sources.calendar.team_authorization`, with `by` and `at`), and even then only the events the person takes part in;

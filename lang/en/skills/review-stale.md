...
4. Re-stamp the note: `generated: { by: {{agent}}, at: <ISO 8601 datetime with its UTC offset> }`, and `stale_after` moved to today plus the months the vault's `stale_policy` (in brain-kit.config.json) sets for the note's folder. Never write `verified`.

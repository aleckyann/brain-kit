...
- Fixture TAXONOMY is English too, and this is not cosmetic. A fixture vault uses `people/`, `projects/`, `decisions/`, `pending/`, `core/`, `memory/`, `attachments/`, never the reference vault's own Portuguese folder names. The kit's claim is that the taxonomy is configuration, so a test suite written in one adopter's taxonomy quietly asserts the opposite, and every later fixture copies whatever the first one did. A non-English taxonomy appears ONLY where it is the thing under test, and every such place says so where its format allows. A JSON fixture cannot say it in itself: the configuration validator rejects unknown top-level keys, so both `$comment` and `_comment` come back as errors, and an earlier version of this rule demanded a comment JSON cannot carry. The label therefore lives in the test that loads the fixture, or in a sibling document, and never nowhere. That is a principle, not a count: an earlier version of this rule allowed exactly one exception, which then collided with the pt-BR configuration fixture whose whole purpose is to prove that non-English configuration works, and the implementer resolved my contradiction by leaving that fixture's collection names in English, gutting the part of the claim that carries the most weight. Two places qualify today: the walk test, proving the engine does not care what folders are called, and the pt-BR configuration fixture, proving the configuration accepts them. The second must therefore name its collections in Portuguese too.
- Files under `src/` and `bin/` are pure ASCII, with no exception, and the em dash character (U+2014) is banned everywhere in the repository. A TEST file may carry deliberate non-ASCII ONLY where the non-ASCII is the thing under test, such as an assertion on accented output from the pt-BR pack, and says so in a comment; everywhere else a test builds non-ASCII with `String.fromCharCode`, because a unicode escape has twice materialised in this repository as a literal byte. This rule lived only in dispatch prompts until now, which is why a stray em dash survived in `src/exec.mjs` until a guard written for another purpose found it. An earlier version banned non-ASCII in tests too, which would have forced an assertion on accented output to spell every letter as a character code, unreadable in service of a rule whose purpose is catching ACCIDENTAL non-ASCII.
- The maintainer's pre-push gate is installed by `.githooks/install-gate` (which puts the hook and its engine outside the working tree and points `core.hooksPath` at them) and scans every commit of a push. Never bypass it with `--no-verify`; if it refuses, report the refusal.
...
**A finding is `{ ruler, id, check, level, section, file, line, message }`, and `check` is the field this slice learned it needed.** A rule id is not fine enough to say what was actually asserted: one rule carries several checks, the format grades some of them `must` and others `should`, and a post-processing step that selects by id alone reaches findings it was never meant to touch. `check` is a stable English name for the individual assertion, unique within its rule. House findings carry `ruler`, `id`, `check`, `file`, `line` and `message`, and never `level`.

**A fence belongs to the container that opened it.** State this once and stop patching cases. A fence opened inside a blockquote is closed only by a marker inside that same blockquote, and a fence opened outside one is not closed by a quoted marker. An unclosed fence runs to the end of ITS container, not to the end of the file. This area has been fixed three times in this repository and each fix added a case without adding a model, which is why each looked wider than it was.

**A downgrade may never reach a finding about something being ABSENT.** This is the invariant behind the `check` field, and it outlives any naming. A check name is only as fine as the assertion its author drew around it, and one round drew a single name around both a presence test and a form test, which put the original defect back one level down. Name checks finely, and also assert the invariant directly, so the next name cannot reintroduce it.

**On stub audits, correcting an earlier instruction of mine.** Report both numbers, but the honest one is from whichever variant actually stubs the code the test exercises, not always the id-preserving one. A variant that leaves a function real cannot fail that function's tests, so those tests carry no information in it.

**Code stripping lives in ONE module, `src/markdown.mjs`, imported by both rulers.** Fenced and inline code must be removed before any rule reads a heading or a link, and each ruler having its own copy is how the two came to disagree about what a fence is. A fence opens on a line with at most three leading spaces carrying at least three backticks or at least three tildes, closes only on a matching marker at least as long, and an unclosed fence runs to the end of the file. A four-space indented block is code too, and a fence-shaped line inside one opens nothing. Inline code spans are removed by backtick-run length, so a double-backtick span hides its contents rather than unwrapping them. The project already paid for this lesson once: the phase 0 push gate needed five rounds of fixes because its logic lived in many places that each had to remember, and duplicated-for-independence is worth nothing when the copies must agree in order to be correct.
...
- Section 4.1, on types, WHOLE: "consumers MUST tolerate unknown types gracefully, typically by treating them as generic concepts." This is the direct authority for a type enumeration being a house rule and never a specification one. An earlier version of this list gave only the first half, which is a third unmarked elision by the same author in one day.
- Section 5.2, in its own words rather than through section 11's cross-referencing bullet: "Consumers MUST treat a bare mapping as a one-element list."
- The format writes cross-references with a section sign, which is not ASCII. Spell it "section" inside a quotation and DECLARE that substitution where you quote, rather than making it silently. A substitution you do not announce is the same class of error as an elision you do not mark.
...
**One consistency the module owes its own citation.** It quotes section 5 as binding "every timestamp-valued key", then checks the format of only two of them. `verified[].at` is tested for presence and not for form, and `sources[].last_modified` is not tested at all. Either the module validates every timestamp-valued key it can reach, or it stops quoting a sentence wider than what it enforces. It validates them all: the fix is cheap, and a citation that claims more than the code does is the same defect as a `must` claimed on a `should`, pointed the other way.
...
- `--only-problems` prints only the GROUPS that have findings. Not rules: there is no per-rule inventory of what was checked, so a rule with nothing to say is indistinguishable from a rule that did not run, and hiding on that basis would claim knowledge the command does not have. An earlier version of this plan said rules twice and is the reason the tool and the plan disagreed. `--json` prints one object with `findings`, `stale`, `counts`, `parserLimits` and the two ruler names, and prints nothing else on stdout.
- The three groups are a PARTITION, not three filters. Every finding lands in exactly one, and a finding whose level is neither of the two the format defines is itself a defect in the tool: report it loudly rather than letting it fall between the groups, where it would vanish from the screen and from the counts while still turning the exit code red.
- The closing verdict line, if there is one, is computed from the SAME predicate as the exit code. A last line saying the vault is fine above a red build is the one failure this command exists to prevent, and it is worse than no verdict at all.
- Every user-facing string, finding messages included, comes from the language pack chosen by the vault's own configuration. A report that frames its sections in one language and writes its findings in another is not bilingual, it is broken.
- A path argument that is neither a vault root nor inside the vault it names is an error with exit `2`, never a silent climb to some enclosing vault. Answering a typo with a confident report about a scope nobody asked for is a confident wrong answer, which is the class of failure this command ranks above all others.
...
**How the contract clauses are PROVEN, replacing an instrument this slice found insufficient.** For each clause this task owns, mutate exactly that clause in a copy of the tree and confirm a named test fails. A mutation must be verified to actually CHANGE BEHAVIOUR, or it proves nothing: on one task here, deleting an explicit not-a-number guard was a no-op, because the comparison beside it already returned false for that value, and the implementer caught its own dead mutation and found the one that genuinely breaks the clause. A test that survives a mutation which changed nothing has not been tested. The clauses: the single walk, everything included in the set the link checker resolves against, the reader's normalisation, and the read cache. A stub audit, which guts a whole module and counts survivors, came back clean while all four of these had no test at all: gutting a function is not the same as perturbing a clause. Keep the stub audit as a cheap smoke check and never as the evidence. And DERIVE the clause list from the code as it stands when the round ENDS, never from the clauses a previous review named. A round that mutates only what it was told to mutate inherits that review's blind spots, and the clauses a round introduces are exactly the ones no earlier review could have named: on one task here, twenty-one mutations found five undefended clauses, and one of them was the round's own headline safety property. Specifically, the single walk is proven by COUNTING READS, not by spying on an injected seam, because a spy cannot see a call that bypasses the seam and the real function sits one line away in module scope.
...
- [ ] **Step 1:** write the failing tests first, including one asserting that no file under `src/` contains a non-ASCII byte or a word from a fixed Portuguese list, so this class cannot return silently. That guard must assert it actually SCANNED something, and must see files the version-control index does not yet know about, because the order in which work is done means a brand new source file is exactly the one it would miss. A guard that can pass while scanning nothing is worse than no guard, since it also stops anyone looking.
...
- [ ] **Step 1: Write the parity test.** It is skipped unless `BRAIN_KIT_PARITY_VAULT` names a directory. Two facts settled after this plan was written change what the comparison must do. The reference vault has NO configuration file, because the command that adopts an existing vault belongs to a later slice, so the test writes one into a temporary directory and points the validator at the vault through it rather than writing anything into the vault itself. And the specification requires every timestamp to carry an explicit offset while that vault writes plain dates throughout, so without `validate.timestamp_deviation` set to allow, a single expected divergence would appear once per note and drown every other signal. When set, it runs the original validator (`node scripts/validate-okf.mjs .` inside that directory) and the new one (`brain-kit validate` with a configuration file that reproduces the house rules of that vault), and compares the two verdicts file by file. A difference is a failure unless it appears in the accepted-divergence list that the test loads from `docs/validator-parity.md`.

- [ ] **Step 2: Run it against the original vault** and write `docs/validator-parity.md`: the date, the two verdicts, every divergence with its cause, and whether each is a deliberate improvement or a defect to fix. The four defects listed at the top of this plan are expected to appear as divergences, each one an improvement. Any other divergence is a defect in the new validator and must be fixed before this task closes, not explained away. Keep the document free of household data, and understand that a FILE PATH is household data: a folder name reveals a taxonomy and a filename reveals a person or a client. So the document carries COUNTS and RULE IDS and nothing else, describes the vault only as "the reference vault", quotes no note content, and names no path. Where an example is unavoidable, invent one in the fixture taxonomy. This project has leaked its own household five separate times in one day, every time through an artifact built to test or describe the public thing, and never through the paths the design guards. This document is that kind of artifact.
...
- `npm test` passes. The suite began this slice at 60 tests and the estimate written here of roughly 110 was wrong by a factor of three; it stands above 350, almost all of the excess coming from rounds that closed defects which would have told a person their own writing was broken when it was not.

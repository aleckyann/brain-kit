...
### Slice closed on 22/09/2026, and what it hands on

Closed at `4406c12`, 930 tests. The last fix round had no scoped re-review of its own. That is a ruling rather than an omission: I verified every leak-bearing behaviour it touched myself, on throwaway repositories and by real push, and the next slice's first task rewrites the adopter gate's scanning path, so its review covers this surface again. Verified: a committed `.env` and `.aws/credentials` are found and refused by the adopter's gate; an ignored local `.env` passes because it will never be pushed; a credential in another field of the configuration is found; a pattern for the owner's own domain no longer flags the owner's address in the configuration and still finds the same domain in a note; an accented configured pattern matches again; and the maintainer's gate refuses an accented name in content and in a file name, and a pattern on the last line of a sixty-megabyte file, while a clean push says it ran.

Handed to slice D, where `init` installs the adopter's hook into a real vault:

- **Blocking.** The adopter's gate checks the working tree and never the objects a push carries: history, a tip hidden by an uncommitted edit, and a branch other than the one checked out all reach the remote. `init` must not install the hook until it scans what the push carries.
- The adopter's gate reads file content, not file names, so a note NAMED after a configured pattern passes; a commit message is not read either. Both close with the object-level scan above if it reuses the maintainer's channels, which it should.
- A vault that runs `npm install` without ignoring `node_modules/` is refused on its first push, told to rotate credentials that do not exist, because the shipped package itself contains text matching its own generic shapes. `init` must write `node_modules/` into `.gitignore`, the template's "install it as a dependency of this vault" message contradicts this plan's rule that a vault carries no package, and the package should stop matching its own shapes.

Declared limits of both gates, not defects: compressed content (a zip, a Flate stream in a PDF), a name split across lines, a JSON-escaped credential, and text in decomposed Unicode form. The last is cheap to close by normalising pattern and content to the same form, and belongs to the next change to the scanner.

**A method ruling for slice C onwards, made because this slice's verification cost more than its implementation.** The last round spent most of two hours and forty minutes running one hundred and twenty-four clause mutations, each against the whole suite. Clause-by-clause mutation stays mandatory where a silent pass can leak or corrupt: the gates, the scanner, the secrets rule, anything that decides an exit code. Elsewhere, message wording, output formatting and report layout, it is replaced by ordinary tests plus the two questions. The rule that decides which side a clause is on: could this clause, deleted, let something reach a remote, or let a run exit zero that should not? If yes, mutate it. If no, test it and move on.

...
// The turn limit every round runs under, read the way the round reads it
// (roundTurns): the whole number curate.max_turns sets, the default when the
// key is absent, or none when it is null (phase 5a, ruling R-A3). A value no
// round can run with (0, a fraction, anything the schema refuses) fails.
function turnCap(ctx) {
  const id = 'turn-cap';
  const inputs = curateInputs(ctx, id);
  if (inputs.result) return inputs.result;
  if (inputs.disabled) return curateDisabled(ctx, id);
  const setting = 'curate.max_turns';
  const turns = roundTurns(inputs.config);
  if (turns !== null && !(Number.isInteger(turns) && turns >= 1)) {
    return { id, status: 'fail', messageKey: 'doctor.turn_cap.unusable', params: { setting, file: CONFIG_FILENAME, value: JSON.stringify(turns) } };
  }
  if (turns === null) return { id, status: 'ok', messageKey: 'doctor.turn_cap.none', params: { setting, file: CONFIG_FILENAME } };
  if (inputs.config.curate?.max_turns === undefined) return { id, status: 'ok', messageKey: 'doctor.turn_cap.default', params: { turns, setting, file: CONFIG_FILENAME } };
  return { id, status: 'ok', messageKey: 'doctor.turn_cap.set', params: { turns, setting, file: CONFIG_FILENAME } };
}
